...
export default function UpdateForm({ config, formElements, withUpload = false }) {
  ...
  useEffect(() => {
    if (current) {
      ...
      ['employee', 'client', 'invoice', 'paymentMode', 'createdBy'].forEach((fieldName) => {
        if (newValues[fieldName] && typeof newValues[fieldName] === 'object' && newValues[fieldName]._id) {
          newValues = {
            ...newValues,
            [fieldName]: newValues[fieldName]._id,
          };
        }
      });
      form.resetFields();
